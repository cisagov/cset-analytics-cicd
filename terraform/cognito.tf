...
resource "aws_cognito_user_pool_client" "client" {
  ...
  callback_urls                        = ["https://${module.alb.alb_dns_name}", "https://${aws_route53_record.domain.name}"]
  ...
  logout_urls                          = ["https://${module.alb.alb_dns_name}", "https://${aws_route53_record.domain.name}"]
